...
double PMUtils::getCoverage(octomap::Pointcloud target, octomap::Pointcloud reference, double gap)
{
    //double gap = 0.004;
   long int correspondences=0;
   
   octomap::Pointcloud::iterator it_reference;
   octomap::Pointcloud::iterator it_target;
   
   for(it_reference = reference.begin(); it_reference != reference.end(); it_reference++){
     for(it_target = target.begin(); it_target != target.end(); it_target ++)
     {
       if ( (fabs(it_reference->x()-it_target->x())<gap ) 
           && ( fabs(it_reference->y()-it_target->y())<gap ) 
           && ( fabs(it_reference->z()-it_target->z())<gap ) )
       {
            correspondences ++;
            break;
       }
...
void PMUtils::saveOctomapAsText(octomap::ColorOcTree* tree, std::string file_name)
{
    ...
    if (tree){
        ...
        //tree->setBBXMax(x_max);
        //tree->setBBXMin(x_min);
        //tree->expand();
        //cout << "\n bouning box " << tree->getBBXBounds() << endl;
        //cout << "\n resolution " << tree->getResolution() << endl;
        for (octomap::ColorOcTree::leaf_iterator it = tree->begin_leafs(), end = tree->end_leafs(); it!=end; ++it){
            ...
            if (tree->isNodeOccupied(*it)){
                occu_voxels++;
                output_file << p(0) <<" " << p(1) << " " << p(2) << " " << (it->getOccupancy()) << std::endl;
            }
            else{
                free_voxels++;
                output_file <<p(0) <<" " << p(1) << " " << p(2) << " " << (it->getOccupancy()) << std::endl;
   ...
   //delete ot;
   //delete p;
   std::cout << "Octomap saved as text \n\t occupied voxels = " << occu_voxels << std::endl;
   std::cout << "\n\t free voxels = " << free_voxels << std::endl;
}
