 ...
 */ 
  ...
  if(file.is_open()){
    ...
    for(std::string line_string; std::getline(file, line_string);)   //read stream line by line
    {
        std::istringstream input_line(line_string);
            
        float x, y, z;
        input_line >> x >> y >> z;       //now read the whitespace-separated floats
